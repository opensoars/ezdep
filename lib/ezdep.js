...
Ezdep.prototype.checkSingleDep = function (single_key, single_value) {
  ...
  // @TODO handle succes and fail
  if (is('string', type)) {
    // @TODO put this evaluation in method so we can call it in the is('array')
    // clause.
    if (is(type, data)) {
    ...
    // The passed cb should be the method described above
    // So ezdep can allow multiple data types easily
    type.forEach(function (single_type) {
  ...
  // @TODO below should go in a custom method: checkCustom
  ...
  if (customs) {
    for (var custom_key in customs) {
      if (customs.hasOwnProperty(custom_key) &&
        ...
        function done(passed, msg) {
          console.log('@' + self.current_dep + ':' + custom_key, 'passed:', passed, 'msg:', msg);
        }

        var custom_results = customs[custom_key].call(self, data, done);
 ...
 * @return {boolean} - Whether `data` is of type `type`
 */
